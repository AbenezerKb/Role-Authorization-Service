...
// @Security	 BasicAuth
func (u *user) UpdateCorporateUserRoleStatus(ctx *gin.Context) {
	updateStatusParam := dto.UpdateUserRoleStatus{}
	err := ctx.ShouldBind(&updateStatusParam)
	if err != nil {
		err := errors.ErrInvalidUserInput.Wrap(err, "invalid input")
	...
	corporateID, err := uuid.Parse(ctx.Param("corporate-id"))
	if err != nil {
		err := errors.ErrInvalidUserInput.Wrap(err, "invalid role id")
		u.logger.Info(ctx, "invalid role id", zap.Error(err), zap.Any("role id", ctx.Param("id")))
		_ = ctx.Error(err)
	...
	if err := u.userModule.UpdateCorporateUserRoleStatus(ctx, updateStatusParam, corporateID, roleId, userId); err != nil {
		_ = ctx.Error(err)
		return
	}
