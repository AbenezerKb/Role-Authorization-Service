...
func (u *user) UpdateUserRoleStatus(ctx *gin.Context) {
	...
	if err := u.userModule.UpdateUserRoleStatus(ctx, updateStatusParam, ctx.GetString("x-tenant"), roleId, userId); err != nil {
		_ = ctx.Error(err)
		return
	}
...
// SystemUpdateUserRoleStatus updates corporate user's role status
// @Summary      updates tenants user's role status
// @Tags         users
// @Accept       json
// @Produce      json
// @param status body dto.UpdateUserRoleStatus true "status"
// @param 		 tenant_id 	path string true "role id"
...
// @Failure      401  {object}  model.ErrorResponse "unauthorized"
// @Failure      403  {object}  model.ErrorResponse "access denied"
// @Router       tenants/{tenant_id}/{user_id}/roles/{role_id}/status [patch]
// @Security	 BasicAuth
func (u *user) SystemUpdateUserRoleStatus(ctx *gin.Context) {
	updateStatusParam := dto.UpdateUserRoleStatus{}
	err := ctx.ShouldBind(&updateStatusParam)
	if err != nil {
		err := errors.ErrInvalidUserInput.Wrap(err, "invalid input")
	...
	if err := u.userModule.UpdateUserRoleStatus(ctx, updateStatusParam, ctx.Param("tenant_id"), roleId, userId); err != nil {
		_ = ctx.Error(err)
		return
	}
