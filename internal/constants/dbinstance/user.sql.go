...
func (q *DBInstance) UpdateUserRoleStatus(ctx context.Context, tenant,
	...
	UPDATE tenant_users_roles tur SET status ='%s' FROM _tenants,_user WHERE tur.role_id='%s' AND tur.deleted_at IS NULL AND
	tur.tenant_id=_tenants.id AND tur.user_id=_user.id RETURNING tur.id;`, tenant, serviceId, userId, serviceId, status, roleId))
		if err != nil {
			return err
		}
	}
